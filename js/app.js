...
(function () {
    ...
    function certificateView() {
        ...
        if (modulesDone() !== COURSES.length || !state.final.quiz.passed) {
            ...
            edge.appendChild(node("div", "message-chip", "FORMATION VIBECODING · CERTIFICAT"));
            edge.appendChild(node("h2", "cert-title", "Certificat de complétion"));
            edge.appendChild(node("p", "cert-name", profileName()));
            var d = new Date();
            edge.appendChild(node("p", "", "a terminé avec succès la formation « de l'idée au site en ligne », qui couvre la conception, l'UI/UX, le HTML, le CSS et le vibecoding."));
            edge.appendChild(node("p", "trace-note", "Délivré le " + d.getDate() + "/" + (d.getMonth() + 1) + "/" + d.getFullYear()));
